...
因为在安装 Floodgate 后，基岩版玩家的 UUID 将会和 Java 版玩家有很大不同，若是遇到重名，则会出现一个 ID 拥有两个完全不同的 UUID，这可能会发生一些无法预知的错误，而使用"."前缀是因为 Java 正版玩家名中不可使用"."
...
于是将前缀修改为诸如"BE\_"这类的合法字符即可解决第二个问题
